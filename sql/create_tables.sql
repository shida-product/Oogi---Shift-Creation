...
-- ============================================================
-- 1. ogi_staff テーブル（M2拡張カラム込み）
-- ============================================================
CREATE TABLE ogi_staff (
...
-- ============================================================
-- 2. ogi_shift_requests テーブル（希望休）
-- ============================================================
CREATE TABLE ogi_shift_requests (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_ogi_shift_requests_date ON ogi_shift_requests(date);
CREATE INDEX idx_ogi_shift_requests_staff_date ON ogi_shift_requests(staff_id, date);

-- ============================================================
-- 3. ogi_shift_assignments テーブル（シフト生成結果）
-- ============================================================
-- CSV出力と1:1対応する構造に設計
--   attendance_type → CSV「勤怠区分」にそのまま出力
--   work_pattern    → CSV「勤務パターン」にそのまま出力
CREATE TABLE ogi_shift_assignments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- ============================================================
CREATE TABLE ogi_monthly_settings (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TRIGGER ogi_staff_updated_at
  BEFORE UPDATE ON ogi_staff
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER ogi_shift_requests_updated_at
  BEFORE UPDATE ON ogi_shift_requests
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- ============================================================
-- 6. 初期データ投入 (データ移行時は実行しないためコメントアウト)
-- ============================================================
/*
-- 月別公休数
INSERT INTO ogi_monthly_settings (year_month, employee_days_off) VALUES
...
-- スタッフ（薬剤師）
INSERT INTO ogi_staff (employee_no, name, display_order, role, staff_type, assigned_store, work_conditions, store_priority) VALUES
